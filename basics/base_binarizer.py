...
class BaseBinarizer:
    ...
    def check_coverage(self):
        ...
        if ph_idx_occurred != ph_idx_required:
            ...
            allow_missing = bool(self.binarization_args.get('allow_missing_phonemes', False))
            if allow_missing:
                # Save a summary file and continue with a warning
                try:
                    summary_path = self.binary_data_dir / 'missing_phonemes.json'
                    # Convert tuples to lists for JSON serialization
                    serializable = [list(p) if isinstance(p, tuple) else p for p in missing_phones]
                    with open(summary_path, 'w', encoding='utf-8') as f:
                        json.dump(serializable, f, ensure_ascii=False, indent=2)
                    print(f"| warning: {len(missing_phones)} phonemes not covered; saved to '{summary_path}'")
                except Exception as e:
                    print(f"| warning: failed to save missing phoneme summary: {e}")
                print("| missing phonemes:", missing_phones)
            else:
